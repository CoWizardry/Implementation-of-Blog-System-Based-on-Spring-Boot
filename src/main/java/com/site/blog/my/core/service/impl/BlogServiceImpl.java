...
import com.site.blog.my.core.controller.DTO.BlogDTO;
import com.site.blog.my.core.controller.DTO.BlogDTO2;
...
public class BlogServiceImpl implements BlogService {
    ...
    public PageResult getBlogsForIndexPage(int page) {
        ...
        List<Blog> blogList = blogMapper.findBlogList(pageUtil);
        List<BlogDTO> blogDTOS = getBlogListVOsByBlogs(blogList);
        int total = blogMapper.getTotalBlogs(pageUtil);
        PageResult pageResult = new PageResult(blogDTOS, total, pageUtil.getLimit(), pageUtil.getPage());
        return pageResult;
    }

    @Override
    public List<BlogDTO2> getBlogListForIndexPage(int type) {
        List<BlogDTO2> blogDTO2s = new ArrayList<>();
        List<Blog> blogs = blogMapper.findBlogListByType(type, 9);
        if (!CollectionUtils.isEmpty(blogs)) {
            for (Blog blog : blogs) {
                BlogDTO2 blogDTO2 = new BlogDTO2();
                BeanUtils.copyProperties(blog, blogDTO2);
                blogDTO2s.add(blogDTO2);
            }
        }
        return blogDTO2s;
    }
    ...
    private BlogDTO0 getBlogDetailVO(Blog blog) {
        if (blog != null && blog.getBlogStatus() == 1) {
            ...
            //分类信息
            blogDTO0.setBlogCategoryIcon(blogCategory.getCategoryIcon());
            if (StringUtils.hasText(blog.getBlogTags())) {
                //标签设置
                List<String> tags = Arrays.asList(blog.getBlogTags().split(","));
                blogDTO0.setBlogTags(tags);
            }
            //设置评论数
            Map params = new HashMap();
            params.put("blogId", blog.getBlogId());
            params.put("commentStatus", 1);//过滤审核通过的数据
            blogDTO0.setCommentCount(blogCommentMapper.getTotalBlogComments(params));
            return blogDTO0;
        }
        return null;
    }

    private List<BlogDTO> getBlogListVOsByBlogs(List<Blog> blogList) {
        ...
        if (!CollectionUtils.isEmpty(blogList)) {
            ...
            for (Blog blog : blogList) {
                ...
                if (blogCategoryMap.containsKey(blog.getBlogCategoryId())) {
                    blogDTO.setBlogCategoryIcon(blogCategoryMap.get(blog.getBlogCategoryId()));
                } else {
